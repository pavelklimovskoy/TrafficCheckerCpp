...
bool Checker::IsMatch(Seqment s1, Seqment s2)
{
	...
	__int64 length = pow((s2._p1._x - s1._p1._x), 2) + pow((s2._p1._y - s1._p1._y), 2);

	if (length <= pow((s1._p1._rad + s2._p1._rad), 2))
	{
		flag = true;
	}
	return flag;
...
bool Checker::IsAgentRoadMatch(Seqment s, Agent a)
{
	...
	double h = abs(2 * sqrt(polup * (polup - rebro1) * (polup - rebro2) * (polup - rebro3)));
	//h /= rebro1;
